...
```{r timevar_rbf_calibration}
#---- Calibrate using the selected parameters ----#
make_rbf_calibrator = function(dimension) {
  mp_tmb_calibrator(
      spec = mp_euler(timevar_spec),
    , data = obsdat
    , traj = "incidence"
    , outputs = c("beta_thing","incidence"), # The "Reporting List"
    ## -----------------------------
    , tv = mp_rbf("beta_thing", dimension)
    # STARTING VALUE
    , par = list(
      beta_thing = mp_normal(1, 2.0)
    )
  )
}

calibrator <- make_rbf_calibrator(dimension = 4)

opt = mp_optimize(calibrator)
